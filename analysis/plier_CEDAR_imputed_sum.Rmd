...
1. For some interested LVs, we did ORA enrichment analysis. 
...
2. For the pairs have p-value < 0.05 in resampling, we checked the effect size correlation could be due to reverse causality.

We first used all SNPs associated with traits(pval<5E-8). We computed the IVW estimator and the p-values of the estimator. When computing the IVW, we exchanged the beta.outcome and beta.exposure. We also fitted the effectsize with beta.exposure ~ 0 + beta.outcome to compare with the IVW results.
...
**The first four columns are the results for LV -> traits(using SNPs have fdr < 0.2), the suffix "_reverse" are the results for traits -> LV (using all candidate SNPs after LD Clumping), the suffix "_rremove" are the results for traits -> LV (using all candidate SNPs after LD Clumping but remove the SNPs have fdr < 0.2)**

```{r echo=FALSE}
load("/project2/xinhe/xsun/website/factor_analysis/output/reverse_check_cd15_plier_cedar.rdata")
DT::datatable(sum, options = list(pageLength =5))
```

Since the IVW estimator and the fitting slope differ a lot, we made some scatter plots for some interested pairs.

<figure class="half">
    <img src="https://github.com/xsun1229/factor_analysis/raw/master/output/ivw_debug_T1D_cd15_lv41.jpeg" width="100%">
</figure>
...
```{r echo=FALSE}
load("/project2/xinhe/xsun/website/factor_analysis/output/reverse_check_bcell_plier_cedar.rdata")
DT::datatable(sum, options = list(pageLength =5))
```
...
<figure class="half">
    <img src="https://github.com/xsun1229/factor_analysis/raw/master/output/ivw_debug_EUR.CD_bcell_lv49.jpeg" width="100%">
    <img src="https://github.com/xsun1229/factor_analysis/raw/master/output/ivw_debug_EUR.CD_bcell_lv96.jpeg" width="100%">
    <img src="https://github.com/xsun1229/factor_analysis/raw/master/output/ivw_debug_EUR.IBD_bcell_lv96.jpeg" width="100%">
    <img src="https://github.com/xsun1229/factor_analysis/raw/master/output/ivw_debug_EUR.UC_bcell_lv80.jpeg" width="100%">
</figure>
...
```{r echo=FALSE}
load("/project2/xinhe/xsun/website/factor_analysis/output/reverse_check_thy_plier_cedar.rdata")
DT::datatable(sum, options = list(pageLength =5))
```
...
```{r echo=FALSE}
load("/project2/xinhe/xsun/website/factor_analysis/output/reverse_check_plt_plier_cedar.rdata")
DT::datatable(sum, options = list(pageLength =5))
```
...
```{r echo=FALSE}
load("/project2/xinhe/xsun/website/factor_analysis/output/reverse_check_tcell_plier_cedar.rdata")
DT::datatable(sum, options = list(pageLength =5))
```
...
```{r echo=FALSE}
load("/project2/xinhe/xsun/website/factor_analysis/output/reverse_check_cd14_plier_cedar.rdata")
DT::datatable(sum, options = list(pageLength =5))
```
