---
title: "Effect Size Consistency Tests (SNP FDR cutoff: 0.1, PEER regressed)"
author: "XSun"
date: "2022-10-04"
...
2. For the pairs have multiple snps passing FDR < 0.1, do ECT

# Results

## The number of SNPs passing FDR < 0.1 in association tests
...
for (t in 1:length(type)) {
  ...
  load(paste0(folder_path,"results/number_snp_fdr01_allpairs.rdata"))
  
  count_vector <- c()
  for (i in 1:length(snp_num_fdr01)) {
    count_vector <- c(count_vector,snp_num_fdr01[[i]])
  }
  ...
  p[[t]] <- ggplot(summary, aes(x = count_vector, y =Freq)) +
    ...
    labs(x = "Number of SNPs pass FDR < 0.1 for each pair",
         y = "Count") +
...
for (i in 1:length(path)){
  ...
  load(paste0(folder_path,"results/ECT_summary_table_fdr01.rdata"))
...
for (i in 1:length(type)){ 
  ...
  file_ect_results <- paste0("/project2/xinhe/xsun/CEDAR/",path[i],"/8.regress_out_peer/2peer/2.ECT/results/ECT_results_fdr01.rdata")
  load(file_ect_results)
  
  file_ect_summary <- paste0("/project2/xinhe/xsun/CEDAR/",path[i],"/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
...
### Results for all pairs have >=5SNPs
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/CEDAR/4.B_cell_kegg/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/CEDAR/6.CD14_positive_monocyte_kegg/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
DT::datatable(df_sum, options = list(pageLength =5))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/CEDAR/8.CD15_positive_leukocyte_kegg/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
DT::datatable(df_sum, options = list(pageLength =5))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/CEDAR/16.platelet_kegg/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
DT::datatable(df_sum, options = list(pageLength =5))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/CEDAR/10.T_cell_kegg/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
DT::datatable(df_sum, options = list(pageLength =5))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
load("/project2/xinhe/xsun/CEDAR/14.thymocyte_kegg/8.regress_out_peer/2peer/2.ECT/results/ECT_summary_table_fdr01.rdata")
DT::datatable(df_sum, options = list(pageLength =5))
```
