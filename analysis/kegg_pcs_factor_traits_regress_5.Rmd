...
For the pairs have more than 2 SNPs with FDR <0.2 (1064 pairs), we also tested the effectsize correlation(effect size in GWAS and effect size in factos-association). We fitted the effectsize using: effectsize in GWAS ~ 0 +effectsize in factor-association. We recorded the p-values and r-squareds in the fitting.
...
There are 218 pairs show sign consistency (have p-values < 0.05 in resampling). There are 81 pairs at p-value < 0.01). 
...
After deleting the pairs whose 'num_snp_noMHC' == 0, we got 759 pairs. There are 134 pairs show sign consistency (have p-values < 0.05 in resampling). There are 38 pairs at p-value < 0.01).  
...
<figure class="half">
    <img src="https://github.com/xsun1229/factor_analysis/raw/master/output/effectsizesum_0.2_keeg_pc_reg_5.jpeg" width="60%">
</figure>
