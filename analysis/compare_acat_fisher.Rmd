...
For each factor - trait pair, we counted the number of SNPs passing different FDR cutoffs (0.05, 0.1, 0.2) from real data and compare the number from random data using Fisher Exact test. The p-values of the tests were recorded. 
...
load("/project2/xinhe/xsun/CEDAR/global_analysis_FDR_corrected/results/summary_acat_fisher.rdata")
...
for (i in 1:length(type)) {
  ...
  plotdata <- data.frame(observed = -log10(sort(as.numeric(as.character(df$fisher_p_01)))),
                         expected = -log10(ppoints(nrow(df))))
  #  
...
for (i in 1:length(type)) {
  ...
  plotdata <- data.frame(observed = -log10(sort(as.numeric(as.character(df$fisher_p_02)))),
                         expected = -log10(ppoints(nrow(df))))
  #  
...
There are convergence signal when using SNP FDR < 0.2. So we compared the FDR with the corrected FDR from ACAT.
...
for (i in 1:length(type)) {
  ...
  p[[i]] <- ggplot(df, aes(x=acat_fdr_correct, y=as.numeric(fisher_p_02))) +
  geom_point() +
    ggtitle(type[i]) + theme(plot.title = element_text(hjust = 0.5)) +
    labs(x = "corrected ACAT FDR",
         y = "Fisher FDR") +
