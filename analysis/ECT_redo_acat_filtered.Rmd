---
title: "Effect Size Consistency Tests (SNP FDR < 0.2, FDR corrected, ACAT FDR < 0.1)"
author: "XSun"
date: "2022-11-17"
...
2. For the pairs have multiple snps (>=3) passing FDR < 0.2 && ACAT FDR < 0.1, do ECT
...
for (i in 1:length(path)){
  ...
  load(paste0(folder_path,"results/ECT_summary_table_acat_filtered.rdata"))
...
for (i in 1:length(type)){ 
  ...
  file_ect_summary <- paste0("/project2/xinhe/xsun/CEDAR/",path[i],"/9.pcs_40genes_rerun/2.ECT/results/ECT_summary_table_acat_filtered.rdata")
...
## Checking the lost top1 pair

"pwy124_pc1" - EUR.CD

ACAT p-value: 0.079, ACAT FDR: 0.1036 (shall we rounded up two desmos place to keep this pair) 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE,fig.height=5, fig.width=6}
 ...
 plotdata <- data.frame(observed = -log10(sort(as.numeric(as.character(dat$pval.exposure)))),
